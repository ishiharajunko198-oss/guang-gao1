...
const RequestMaterials: React.FC<Props> = ({isOpen, onClose}) => {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const API_PATH = "/v3/api/inquiry-info/jp/collect"
    const request_body = {
      name,
      nameSpelling: kana,
      companyName: company,
    ...
    fetch(API_PATH, {
      ...
      .catch((error) => {
        ...
        setIsSubmitting(false);
        setErrors({
          agreed: "送信に失敗しました。お手数ですが、しばらくしてから再度お試しください。"
        });
      });
  };
              ...
              <div className="space-y-6 text-[12px] text-gray-500 leading-relaxed border-t border-gray-200 pt-8">
                ...
                <p>
                  ...
                  <a href={PRIVACY_POLICY_URL} target="_blank" rel="noopener noreferrer"
                     className="text-jp-blue hover:underline font-bold ml-1">【プライバシーポリシー】</a>
                </p>
                <p className="bg-white/50 p-4 rounded-xl border border-gray-200 flex items-start gap-3">
                  ...
                  <a href={PRIVACY_POLICY_URL} target="_blank" rel="noopener noreferrer"
                     className="text-jp-blue hover:underline ml-1">プライバシーポリシー</a>
                </span>
                </p>
              </div>
            </aside>
              ...
              <form ref={formRef} onSubmit={handleSubmit} className="space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <input name="name" type="text" placeholder="例：田中 太郎"
                    ...
                    {errors.name &&
                      <p className="text-red-500 text-[11px] flex items-center gap-1 font-bold animate-pulse">
                        <AlertCircle
                          size={12}/> {errors.name}</p>}
                  </div>
                  <div className="space-y-2">
                    <div className="flex items-center gap-2">
                    ...
                    <input name="kana" type="text" placeholder="例：タナカ タロウ"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <input name="company" type="text" placeholder="例：株式会社SellerSprite"
                  ...
                  <div className="space-y-2">
                    ...
                    <input name="dept" type="text" placeholder="例：EC事業部（なしの場合は「なし」）"
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <select name="genre"
                            className={`w-full px-5 py-4 bg-gray-50 border rounded-xl outline-none transition-all text-sm appearance-none pr-12 ${errors.genre ? 'border-red-400 ring-2 ring-red-50' : 'border-gray-200 focus:border-jp-cta focus:bg-white'}`}
                            defaultValue="">
                  ...
                  {errors.genre &&
                    <p className="text-red-500 text-[11px] flex items-center gap-1 font-bold animate-pulse"><AlertCircle
                      size={12}/> {errors.genre}</p>}
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <input name="email" type="email" placeholder="例：sellersprite@example.com"
                  ...
                  <div className="space-y-2">
                    ...
                    <input name="tel" type="tel" placeholder="例：03-1234-5678"
                ...
                <div className="space-y-4">
                  <label
                    className="block text-[13px] font-black text-jp-navy">資料請求以外、興味・関心のあるサービス（任意）</label>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {interestOptions.map((option, idx) => (
                      <label key={idx}
                             className="flex items-center gap-3 p-3 bg-gray-50 border border-gray-100 rounded-xl cursor-pointer hover:bg-orange-50 transition-colors group">
                        <input type="checkbox" name="interests" value={option}
                               className="w-4 h-4 accent-jp-cta rounded border-gray-300"/>
                        <span
                          className="text-[12px] sm:text-[13px] text-gray-700 font-medium group-hover:text-jp-navy">{option}</span>
                      </label>
                    ))}
                  </div>
                  <div className="mt-4">
                    ...
                    <textarea name="message" rows={3}
                              ...
                              className="w-full px-5 py-4 bg-gray-50 border border-gray-200 rounded-xl focus:border-jp-cta focus:bg-white outline-none transition-all text-sm resize-none placeholder:text-[10px] xs:placeholder:text-[11px] sm:placeholder:text-sm"></textarea>
                  </div>
                </div>
                ...
                <div className="bg-[#fffbf2] p-6 rounded-2xl border border-[#ffecb3]">
                  <p className="text-[11px] text-gray-500 mb-4 leading-relaxed">
                    ご提供いただいた情報は当社の<a href={PRIVACY_POLICY_URL} target="_blank" rel="noopener noreferrer"
                                                   className="text-jp-blue underline font-bold">プライバシーポリシー</a>に基づき適切に管理いたします。内容をご確認いただき、同意される方のみ送信ボタンを押してください。
                  </p>
                  <label className="flex items-center gap-3 cursor-pointer group">
                    <div className="relative">
                      <input name="agreed" type="checkbox" className="peer sr-only"/>
                      <div
                        className="w-5 h-5 border-2 border-gray-300 rounded bg-white peer-checked:bg-jp-cta peer-checked:border-jp-cta transition-all"></div>
                  ...
                  {errors.agreed &&
                    <p className="text-red-500 text-[11px] mt-2 font-bold animate-pulse">{errors.agreed}</p>}
                </div>
